...
class OauthsinaView(APIView):
    ...
    def get(self, request):
        ...
        if not state:
            state = '/'
        # 3.初始化OAuthQQ对象
        sina = OAuthWb(client_id=settings.WB_CLIENT_ID, client_secret=settings.WB_CLIENT_SECRET, redirect_uri=settings.WB_REDIRECT_URI, state=state)
        # 4.构造qq跳转链接
        login_url = sina.get_qq_url()
        # 5.返回结果
        return Response({'login_url': login_url})
